...
// Test: Start New Chat Keeps Current Service
function testStartNewChatKeepsCurrentService() {
  return new Promise((resolve, reject) => {
    console.log("   Testing Start New Chat keeps current service...");
    ...
    if (allPassed) {
      console.log("   ✅ Start New Chat keeps current service test passed");
      resolve();
    } else {
      reject(new Error("Start New Chat keeps current service test failed"));
    }
  });
}
...
function testLocalStorageClearing() {
  return new Promise((resolve, reject) => {
    ...
    localStorage.setItem(
      `chat-current-${mockUserEmail}`,
      JSON.stringify(mockCurrentChat)
    );
...
function testNewChatButtonVisibility() {
  return new Promise((resolve, reject) => {
    ...
    testCases.forEach((testCase, index) => {
      const shouldShow =
        testCase.chatStarted &&
        !testCase.showSummary &&
        !testCase.chatCancelled;

      if (shouldShow !== testCase.shouldShow) {
        allPassed = false;
        console.log(
          `   ❌ Test case ${index + 1} failed: expected ${testCase.shouldShow}, got ${shouldShow}`
        );
      }
    });
...
if (typeof module !== "undefined" && module.exports) {
  module.exports = {
    testStartNewChat,
    testStartNewChatFromDifferentServices,
    testStartNewChatKeepsCurrentService,
    testLocalStorageClearing,
    testNewChatButtonVisibility,
    testChatHistoryPreservation,
  };
}

// Initialize tests if running in browser
if (typeof window !== "undefined") {
  const runner = new TestRunner();

  runner.addTest("Start New Chat Functionality", testStartNewChat);
  runner.addTest(
    "Start New Chat from Different Services",
    testStartNewChatFromDifferentServices
  );
  runner.addTest(
    "Start New Chat Keeps Current Service",
    testStartNewChatKeepsCurrentService
  );
